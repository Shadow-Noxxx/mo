...
async def auth(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type == "private" and user.id != OWNER_ID:
        return await update.message.reply_text("🚫 This command can only be used in groups or by the bot owner.")

    if chat.type != "private" and user.id != OWNER_ID:
        try:
            member = await chat.get_member(user.id)
            if not member.can_promote_members:
                return await update.message.reply_text("🚫 You must be a group admin with 'Add Admin' rights.")
        except:
            return await update.message.reply_text("🚫 Failed to check admin rights.")

    if len(context.args) != 1:
        return await update.message.reply_text("✅ Usage: /auth <user_id>")

    try:
        user_id = int(context.args[0])
        if user_id not in sudo_users:
            sudo_users.append(user_id)
            save_auth_users(sudo_users)
            await update.message.reply_text(f"✅ Authorized user added: <code>{user_id}</code>", parse_mode=constants.ParseMode.HTML)
        else:
            await update.message.reply_text("⚠️ This user is already authorized.")
    except ValueError:
        await update.message.reply_text("🚫 Invalid user ID.")
  # ✅ correct if you intend it to be a list
...
async def unauth(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(context.args) != 1:
        return await update.message.reply_text("✅ Usage: /unauth <user_id>")
    try:
        user_id = int(context.args[0])
        if user_id == OWNER_ID:
            return await update.message.reply_text("⚠️ You cannot unauth the owner.")
        if user_id in sudo_users:
            sudo_users.remove(user_id)
            save_auth_users(sudo_users)
            await update.message.reply_text(f"❌ Removed <code>{user_id}</code> from authorized users.", parse_mode=constants.ParseMode.HTML)
        else:
            await update.message.reply_text("⚠️ This user is not authorized.")
    except ValueError:
        await update.message.reply_text("🚫 Invalid user ID.")

async def authusers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id != OWNER_ID:
        return await update.message.reply_text("🚫 You are not authorized to view this list.")
    ...
    for i, uid in enumerate(sudo_users, start=1):
        try:
            ...
            text += f"{i}. <code>{uid}</code> (Fetch failed)\n"
    await update.message.reply_text(text, parse_mode=constants.ParseMode.HTML)

async def check_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.edited_message:
        edited = update.edited_message
        if edited.from_user.id not in sudo_users:
            try:
                await context.bot.delete_message(chat_id=edited.chat_id, message_id=edited.message_id)
                mention = f"<a href='tg://user?id={edited.from_user.id}'>{html.escape(edited.from_user.first_name)}</a>"
                await context.bot.send_message(
                    chat_id=edited.chat_id,
                    text=f"{mention} tried to edit a message. Deleted!",
                    parse_mode=constants.ParseMode.HTML
                )
            except Exception as e:
                logger.error(f"Edit deletion error: {e}")
...
async def lowpromote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = update.effective_user.id
    # Allow Lord, Substitute Lords, and Descendants to use this command
    if not (is_admin(await context.bot.get_chat_member(chat_id, user_id)) or is_sudo(user_id)):
        await update.message.reply_text("👮 <b>You must be an admin or sudo user to use this command!</b>", parse_mode="HTML")
        return
...
async def midpromote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not result:
        return
    chat_id, target_user, is_owner = result
    title = " ".join(context.args[1:]) if len(context.args) > 1 else "Senior Admin"
    # Lord or Substitute Lord can always promote, skip privilege check
    if update.effective_user.id != sudo_users["lord" ] and update.effective_user.id not in sudo_users.get("substitute_lords", set()):
        promoter = await context.bot.get_chat_member(chat_id, update.effective_user.id)
        if not is_owner and not getattr(promoter, "can_promote_members", False):
            await update.message.reply_text("🔒 You need promotion privileges to assign this level!")
            return
    try:
        await context.bot.promote_chat_member(
            chat_id=chat_id,
        ...
        try:
            ...
            pass
        await update.message.reply_text(
            f"🛡 {target_user.mention_html()} promoted to <b>Senior Admin</b>!\n"
...
async def fullpromote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not result:
        return
    chat_id, target_user, is_owner = result
    title = " ".join(context.args[1:]) if len(context.args) > 1 else "Head Admin"
    # Only owner, Lord, or Substitute Lord(s) can assign Head Admin
    if not is_owner and update.effective_user.id != sudo_users["lord" , "substitute_lords"] and update.effective_user.id not in sudo_users.get("substitute_lords", set()):
        await update.message.reply_text("👑 Only the group owner, Lord, or Substitute Lord can assign Head Admin status!")
        return
    try:
        await context.bot.promote_chat_member(
            chat_id=chat_id,
        ...
        try:
            ...
            pass
        await update.message.reply_text(
            f"👑 {target_user.mention_html()} promoted to <b>Head Admin</b>!\n"
            f"🏷️ Title: <code>{html.escape(title)}</code>\n"
            f"Rights: FULL ADMIN RIGHTS (can promote others)",
            parse_mode="HTML"
        )
    except Exception as e:
        await update.message.reply_text(f"❌ Failed to promote user:\n<code>{html.escape(str(e))}</code>", parse_mode="HTML")
...
async def demote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Only admins or sudo users can use this command.
    """
    try:
        if update.message.chat.type == "private":
            await update.message.reply_text("🚫 <b>This command can only be used in groups!</b>", parse_mode="HTML")
        ...
        bot_id = (await context.bot.get_me()).id
        bot_member = await context.bot.get_chat_member(chat_id, bot_id)
        if not is_admin(bot_member) or not getattr(bot_member, "can_promote_members", False):
            await update.message.reply_text("🤖 <b>I need <u>Add New Admins</u> permission to demote users!</b>", parse_mode="HTML")
            return
        ...
        # Prevent demoting Lord or Substitute Lords
        if target_user.id == sudo_users["lord"] or target_user.id in sudo_users.get("substitute_lords", set()):
            await update.message.reply_text(
                "🚫 <b>You cannot demote the Lord or Substitute Lord with this command!</b>",
                parse_mode="HTML"
            )
            return
        ...
        # Prevent demoting the bot unless by sudo user
        if target_user.id == bot_id and not is_sudo(user_id):
            await update.message.reply_text(
        ...
        try:
            ...
            await update.message.reply_text(
                f"❌ <b>Failed to demote user (API error):</b>\n<code>{html.escape(str(e))}</code>",
                parse_mode="HTML"
            )
            return

        await update.message.reply_text(
            f"⬇️ <b>{target_user.mention_html()} has been demoted and is no longer an admin.</b>",
            parse_mode="HTML"
        )

        # Auto re-promote if bot itself is demoted (by sudo user only)
        if target_user.id == bot_id and is_sudo(user_id):
            try:
                ...
                # Optionally, send a log message to owner or group
                # await context.bot.send_message(chat_id, "🤖 I was demoted, but I have re-promoted myself with full rights!")
            except Exception as e:
                logging.error(f"Failed to auto-promote self after demotion: {e}")

    except Exception as e:
        try:
            await update.message.reply_text(
                f"❌ <b>Unexpected error in demote command:</b>\n<code>{html.escape(str(e))}</code>",
                parse_mode="HTML"
            )
        except Exception:
            pass
